...
class Comics extends Component{
    ...
    render(){
        return(<div>
            ...
            <hr/>
            {this.state.favorito && (<div style={{background:'aquamarine'}}><h1>{this.state.favorito.titulo}</h1><img src={this.state.favorito.imagen}/></div>)}
            <hr/>
            <button onClick={this.eliminarComic}>Eliminar Comic</button>
            {this.state.comics.map((comic,index) => {
               return( <Comic Comic={comic} key={index} seleccionarFavorito={this.seleccionarFavorito} index={index} eliminarComic={this.eliminarComic}/>);
            })}
        </div>)
    }
}
